...
const UpdateDonnation = () => {
  ...
  const formValidation = () => {
    ...
    const getDonnation = async () => {
      const response = await fetch(
        `http://localhost:8000/donnation/getDonnation/${id}`,
        {
          method: "GET",
        }
      );
        ...
        .then((result) => {
          ...
          axios
            ...
            .then((response) => {
                ...
                navigate(
                  `/donnation/request/${window.localStorage.getItem("id")}`
                );
...
export default UpdateDonnation;
